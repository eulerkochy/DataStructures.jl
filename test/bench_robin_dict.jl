...
include("../src/hash_robin_dict.jl")
...
function plot_helper_add_entries(h::RobinDict, entries::Vector{Pair{K, V}}) where {K, V}
	...
	png(plot(x, y, label = ["Load Factor", "maxprobe"]), "lf_and_max_probe_$(Int(sz/1000))K @ LF $(ROBIN_DICT_LOAD_FACTOR)")
	png(plot(xx, yy, label = ["DIB mean", "DIB variance"]), "dibs_$(Int(sz/1000))K @ LF $(ROBIN_DICT_LOAD_FACTOR)")
end
